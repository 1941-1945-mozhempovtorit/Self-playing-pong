...
while isRunning:
    ...
    if ball.velocity[0] < 0:
        if ball.rect.y + ball_radius < paddleA.rect.y + 40:  # If ball is upper than paddle
            paddleA.move_up(paddle_speed)
        elif ball.rect.y + ball_radius > paddleA.rect.y + 60:  # If ball is lower than paddle
            paddleA.move_down(paddle_speed, res_y)
    # Right paddle (B)
    if ball.velocity[0] > 0:
        if ball.rect.y + ball_radius < paddleB.rect.y + 40:  # If ball is upper than paddle
            paddleB.move_up(paddle_speed)
        elif ball.rect.y + ball_radius > paddleB.rect.y + 60:  # If ball is lower than paddle
